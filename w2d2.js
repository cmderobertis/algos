...
// Access values by key (sometimes key AND index)
...
for (let i = 0; i < pokémon.length; i++) {
    ...
    // curate list of names, split/reverse them, push to reversedNames
    if (pokémon[i].types.length == 1 && pokémon[i].types[0] == 'poison') {
        reversedNames.push(pokémon[i].name.split('').reverse().join(''))
    }

    // print names from reversedNames
